...
void ChipNavigator::display_nagivator()
{
    ...
    dbhandler.show_items("block_system_block", {"block_id", "block_name", "abbreviation", "responsible", "prev", "next"}, key_value_pairs, items, "order by block_id");
...
void ChipNavigator::on_treeWidgetBlock_itemClicked(QTreeWidgetItem *item, int column)
{
    if (!item->parent())
    {
        emit(chip_clicked());
        ...
        REGISTER_NAMING.update_key("{BLOCK_NAME}", block_name);
        REGISTER_NAMING.update_key("{BLOCK_ABBR}", block_id2abbr_[block_id]);
        SIGNAL_NAMING.update_key("{BLOCK_NAME}", block_name);
        SIGNAL_NAMING.update_key("{BLOCK_ABBR}", block_id2abbr_[block_id]);
