...
QString QAESEncryption::encode(const QString &text, const QString &key)
{
    ...
    QByteArray encodeText = encryption.encode(text.toUtf8(), hashKey);
    return encodeText.toBase64().constData();
}

QString QAESEncryption::decode(const QString &encoded, const QString &key)
{
    QAESEncryption encryption(QAESEncryption::AES_256, QAESEncryption::ECB, QAESEncryption::Padding::ISO);
    QByteArray hashKey = QCryptographicHash::hash(key.toUtf8(), QCryptographicHash::Sha256);
    return QString::fromUtf8(encryption.removePadding(encryption.decode(QByteArray::fromBase64(encoded.toUtf8()), hashKey)));
}

QString QAESEncryption::generate_key(int len)
{
